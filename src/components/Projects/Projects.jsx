...
//         throw new Error("Expected an object from the API response");
//       }
...
//       ? p.category === "projects"
//       : p.category?.trim().toLowerCase() === selectedTag.trim().toLowerCase()
// );
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        const merged = Object.entries(rawData).flatMap(
          ([category, projects]) => {
            if (!Array.isArray(projects)) return [];
            return projects.map((project) => ({
              ...project,
